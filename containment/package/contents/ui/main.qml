...
ContainmentItem {
    //// END SIGNALS
    //is touching a vertical view that is in busy state and the user prefers isBusy transparency
    ...
    // Plasma 6: expose theme/units singletons as root properties so
    // that all child QML files can keep using global theme.* and
    // units.* without ReferenceError.
    ...
    property int viewTypeInQuestion: {
        ...
        if ((plasmoid.configuration.alignment === LatteCore.Types.Justify || staticLayout) && background.isGreaterThanItemThickness && (parabolic.factor.maxZoom === 1))
    ...
    property bool windowIsTouching: latteView && latteView.windowsTracker && (latteView.windowsTracker.currentScreen.activeWindowTouching || latteView.windowsTracker.currentScreen.activeWindowTouchingEdge || hasExpandedApplet)
    property bool floatingInternalGapIsForced: plasmoid.configuration.floatingInternalGapIsForced
    property bool hasFloatingGapInputEventsDisabled: root.screenEdgeMarginEnabled && !latteView.byPassWM && !root.inConfigureAppletsMode && !parabolic.isEnabled && (root.behaveAsPlasmaPanel || (root.behaveAsDockWithMask && !root.floatingInternalGapIsForced))
    property bool forceSolidPanel: (latteView && latteView.visibility && LatteCore.WindowSystem.compositingActive && !inConfigureAppletsMode && userShowPanelBackground && ((plasmoid.configuration.solidBackgroundForMaximized && !(hasExpandedApplet && !plasmaBackgroundForPopups) && (latteView.windowsTracker.currentScreen.existsWindowTouching || latteView.windowsTracker.currentScreen.existsWindowTouchingEdge)) || (hasExpandedApplet && plasmaBackgroundForPopups))) || !LatteCore.WindowSystem.compositingActive
    property bool forceTransparentPanel: root.backgroundOnlyOnMaximized && latteView && latteView.visibility && LatteCore.WindowSystem.compositingActive && !inConfigureAppletsMode && !forceSolidPanel && !(latteView.windowsTracker.currentScreen.existsWindowTouching || latteView.windowsTracker.currentScreen.existsWindowTouchingEdge) && !(windowColors === LatteContainment.Types.ActiveWindowColors && selectedWindowsTracker.existsWindowActive)
    property bool forcePanelForBusyBackground: userShowPanelBackground && (normalBusyForTouchingBusyVerticalView || (root.forceTransparentPanel && colorizerManager.backgroundIsBusy && root.themeColors === LatteContainment.Types.SmartThemeColors))
    property bool normalBusyForTouchingBusyVerticalView: (latteView && latteView.windowsTracker && latteView.windowsTracker.currentScreen.isTouchingBusyVerticalView && plasmoid.configuration.backgroundOnlyOnMaximized)
    ...
    property bool mirrorScreenGap: screenEdgeMarginEnabled && plasmoid.configuration.floatingGapIsMirrored && latteView.visibility.mode === LatteCore.Types.AlwaysVisible
    ...
    readonly property bool hasUserSpecifiedBackground: (latteView && latteView.layout && latteView.layout.background.startsWith("/")) ? true : false
    ...
    property bool useThemePanel: noApplets === 0 || !LatteCore.WindowSystem.compositingActive ? true : (plasmoid.configuration.useThemePanel || plasmoid.configuration.solidBackgroundForMaximized)
    ...
    property int maxLength: {
        const maximize = behaveAsPlasmaPanel || (maximizeWhenMaximized && latteView.windowsTracker.currentScreen.existsWindowMaximized);
        if (root.isHorizontal)
    ...
    property bool backgroundShadowsInRegularStateEnabled: LatteCore.WindowSystem.compositingActive && userShowPanelBackground && plasmoid.configuration.panelShadows
    property bool panelShadowsActive: {
        ...
        var forcedNoShadows = (plasmoid.configuration.panelShadows && disablePanelShadowMaximized && latteView && latteView.windowsTracker && latteView.windowsTracker.currentScreen.activeWindowMaximized);
        if (forcedNoShadows)
            return false;
        ...
        if (((plasmoid.configuration.panelShadows && !root.backgroundOnlyOnMaximized) || (plasmoid.configuration.panelShadows && root.backgroundOnlyOnMaximized && !root.forceTransparentPanel)) && !forcedNoShadows)
            return true;
    ...
    function createAppletItem(applet) {
        ...
        // create an AppletItem (appletContainer) and initialize its 'applet'
        // property immediately so that bindings and Component.onCompleted
        // inside AppletItem.qml see the logical Plasma::Applet from the start.
        var appletContainer = appletItemComponent.createObject(dndSpacer.parent, { "applet": applet });
    ...
    function initAppletContainer(appletContainer, applet) {
        ...
        var visual = fastLayoutManager.visualAppletFor(applet);
        console.log("LATTE DEBUG: initAppletContainer applet=", applet, "visual=", visual);

        if (visual) {
            // 1) Für Größenberechnung im ItemWrapper
            appletContainer.appletItem = visual;

            // 2) Visuelles Applet in den Wrapper einhängen
            if (appletContainer.wrapper && appletContainer.wrapper.wrapperContainer) {
                visual.parent = appletContainer.wrapper.wrapperContainer;
                visual.anchors.fill = appletContainer.wrapper.wrapperContainer;
                visual.visible = true;
                console.log("LATTE DEBUG: visual parent set to wrapperContainer, size=",
                            visual.width, "x", visual.height,
                            "visible=", visual.visible);
            } else {
                console.log("LATTE DEBUG: missing wrapper or wrapperContainer for appletContainer",
                            appletContainer, appletContainer.wrapper);
            }
        }
    }
    ...
    Component.onCompleted: {
        ...
        //    console.log("LATTE DEBUG: adding applet from QML:", "i=", i, "id=", applet.id, "plugin=", applet.pluginName);
        //    fastLayoutManager.addAppletItem(plasmoid.applets[i], i);
        //}

        upgrader_v010_alignment();
        console.log("LATTE DEBUG: onCompleted, plasmoid.applets length =", plasmoid && plasmoid.applets ? plasmoid.applets.length : "NO_PROPERTY");
         if (plasmoid && plasmoid.applets && plasmoid.applets.length > 0) {
        var a = plasmoid.applets[0];
        console.log("LATTE DEBUG: first applet object =", a);
        for (var p in a) {
            try {
                console.log("LATTE DEBUG: applet[0] prop", p, "=", a[p]);
            } catch (e) {
                console.log("LATTE DEBUG: applet[0] prop", p, "error:", e);
            }
        }
    }
        fastLayoutManager.restore();
        if (plasmoid && plasmoid.action) {
            var cfgAction = plasmoid.action("configure");
            if (cfgAction) {
                cfgAction.visible = !plasmoid.immutable;
                cfgAction.enabled = !plasmoid.immutable;
            }
        }
    }
    ...
    Containment.onAppletAdded: {
        console.log("LATTE DEBUG: Containment.onAppletAdded:", "id=", applet ? applet.id : "null", "plugin=", applet ? applet.pluginName : "null", "x=", x, "y=", y);
    ...
    Binding {
        ...
        value: screenEdgeMarginEnabled && plasmoid.configuration.hideFloatingGapForMaximized && latteView && latteView.windowsTracker && latteView.windowsTracker.currentScreen.existsWindowMaximized
    }
    ...
    Binding {
        ...
        when: latteView && latteView.positioner && latteView.visibility && ((root.behaveAsPlasmaPanel && latteView.positioner.slideOffset === 0) || root.behaveAsDockWithMask) && !(plasmoid.configuration.floatingGapHidingWaitsMouse && dockContainsMouse)
        value: (hideThickScreenGap && (latteView.visibility.mode === LatteCore.Types.AlwaysVisible || latteView.visibility.mode === LatteCore.Types.WindowsGoBelow) && (plasmoid.configuration.alignment === LatteCore.Types.Justify) && plasmoid.configuration.maxLength > 85)
    }
    ...
    Connections {
        ...
        onAlignmentChanged: {
            ...
            if (latteView.alignment === LatteCore.Types.Justify && previousalignment !== LatteCore.Types.Justify) {
                // main -> justify
                layouter.appletsInParentChange = true;
            ...
            } else if (latteView.alignment !== LatteCore.Types.Justify && previousalignment === LatteCore.Types.Justify) {
                // justify ->main
                layouter.appletsInParentChange = true;
    ...
    Item {
        ...
        opacity: 0
        visible: parent === layoutsContainer.startLayout || parent === layoutsContainer.mainLayout || parent === layoutsContainer.endLayout
        z: 1500
            ...
            states: [
                State {
                    ...
                    PropertyChanges {
                        ...
                        anchors.topMargin: 0
                        anchors.bottomMargin: dndSpacerAddItemContainer.thickMargin
                ...
                State {
                    ...
                    PropertyChanges {
                        ...
                        anchors.leftMargin: 0
                        anchors.rightMargin: 0
                        anchors.topMargin: dndSpacerAddItemContainer.thickMargin
                ...
                State {
                    ...
                    PropertyChanges {
                        target: dndSpacerAddItemContainer
                        anchors.leftMargin: dndSpacerAddItemContainer.thickMargin
                        anchors.rightMargin: 0
                        anchors.topMargin: 0
                        anchors.bottomMargin: 0
                        anchors.horizontalCenterOffset: 0
                ...
                State {
                    ...
                    PropertyChanges {
                        target: dndSpacerAddItemContainer
                        anchors.leftMargin: 0
                        anchors.rightMargin: dndSpacerAddItemContainer.thickMargin
                        anchors.topMargin: 0
                        anchors.bottomMargin: 0
                        anchors.horizontalCenterOffset: 0
                        anchors.verticalCenterOffset: 0
    ...
    Loader {
        ...
        sourceComponent: Rectangle {
            ...
            width: Math.min(latteView.localGeometry.width, root.width)
            //! fixes updating
            height: Math.min(latteView.localGeometry.height, root.height)
            color: "blue"
            border.width: 2
            border.color: "red"
            opacity: 0.35
    ...
    Loader {
        ...
        sourceComponent: Rectangle {
            ...
            //! when view is resized there is a chance that geometry is faulty stacked in old values
            width: Math.min(latteView.effects.inputMask.width, root.width)
            //! fixes updating
            height: Math.min(latteView.effects.inputMask.height, root.height)
            color: "purple"
            border.width: 1
            border.color: "black"
            opacity: 0.2
