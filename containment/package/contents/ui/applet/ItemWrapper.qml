...
Item {
    ...
    readonly property real length: {
        ...
        if (appletItem.isInternalViewSplitter) {
            if (!root.inConfigureAppletsMode)
                ...
                return appletItem.inConfigureAppletsDragging && root.dragOverlay.currentApplet.isInternalViewSplitter && root.dragOverlay.currentApplet === appletItem ? appletMinimumLength : internalSplitterComputedLength;
        }
        if ((isSeparator && appletItem.parabolic.isEnabled) || (isMarginsAreaSeparator && (!root.inConfigureAppletsMode || appletItem.parabolic.isEnabled)))
            return -1;
    ...
    property real appletWidth: (appletItem && typeof appletItem.width === "number") ? appletItem.width : -1
    property real appletHeight: (appletItem && typeof appletItem.height === "number") ? appletItem.height : -1
    property real appletMinimumWidth: (appletItem && appletItem.Layout && typeof appletItem.Layout.minimumWidth === "number")
                                  ? appletItem.Layout.minimumWidth : -1
    property real appletMinimumHeight: (appletItem && appletItem.Layout && typeof appletItem.Layout.minimumHeight === "number")
                                  ? appletItem.Layout.minimumHeight : -1
    property real appletPreferredWidth: (appletItem && appletItem.Layout && typeof appletItem.Layout.preferredWidth === "number")
                                  ? appletItem.Layout.preferredWidth : -1
    property int appletPreferredHeight: (appletItem && appletItem.Layout && typeof appletItem.Layout.preferredHeight === "number")
                                  ? appletItem.Layout.preferredHeight : -1
    property real appletMaximumWidth: appletItem && appletItem.Layout ? appletItem.Layout.maximumWidth : -1
    property real appletMaximumHeight: appletItem && appletItem.Layout ? appletItem.Layout.maximumHeight : -1
    readonly property int proposedItemThickness: appletItem.inMarginsArea && !appletItem.canFillThickness ? Math.max(16, appletItem.metrics.marginsArea.iconSize) : appletItem.metrics.iconSize
    ...
    property int localLengthMargins: isSeparator || appletItem.isMarginsAreaSeparator || !communicator.requires.lengthMarginsEnabled || communicator.indexerIsSupported || isInternalViewSplitter ? 0 : appletItem.lengthAppletFullMargins
    ...
    Component.onCompleted: {
        // Fallback initialization to ensure a non-zero length even if
        // layout bindings have not yet run. This uses the icon size as
        // a reasonable default for applet length and thickness.
        if (layoutLength <= 0) {
            layoutLength = appletItem.metrics.iconSize;
        }
        if (layoutThickness <= 0) {
    ...
    // Plasma 6: always draw the wrapper; don't hide it when the Colorizer is active,
    // otherwise applets may become fully invisible if the colorizer pipeline fails.
    opacity: 1
    ...
    Binding {
        ...
        when: latteView && !appletItem.isAutoFillApplet
        value: {
            if (applet && (appletMaximumLength < appletItem.metrics.iconSize || appletPreferredLength > appletItem.metrics.iconSize || appletItem.originalAppletBehavior)) {
                ...
                if (appletMaximumLength >= 0 && appletMaximumLength < appletItem.metrics.iconSize)
                    ...
                    return appletMinimumLength;
                else if ((appletPreferredLength > appletItem.metrics.iconSize) || (appletItem.originalAppletBehavior && appletPreferredLength > 0))
    ...
    Binding {
        ...
        value: {
            ...
            if (appletItem.isInternalViewSplitter) {
                ...
                } else if (applet && (appletMaximumLength < appletItem.metrics.iconSize || appletPreferredLength > appletItem.metrics.iconSize || appletItem.originalAppletBehavior)) {
                    //this way improves performance, probably because during animation the preferred sizes update a lot
                    if (appletMaximumLength > 0 && appletMaximumLength < appletItem.metrics.iconSize)
                        return false;
                    else if (appletMinimumLength > appletItem.metrics.iconSize)
    ...
    Loader {
        ...
        active: appletItem.applet && appletItem.environment.isGraphicsSystemAccelerated && !appletColorizer.mustBeShown && (appletItem.myView.itemShadow.isEnabled && !appletItem.communicator.indexerIsSupported)
        ...
        states: [
            State {
                ...
                PropertyChanges {
                    ...
                    anchors.topMargin: 0
                    anchors.bottomMargin: _wrapperContainer.tailThicknessMargin
            ...
            State {
                ...
                PropertyChanges {
                    ...
                    anchors.leftMargin: 0
                    anchors.rightMargin: 0
                    anchors.topMargin: _wrapperContainer.tailThicknessMargin
            ...
            State {
                ...
                PropertyChanges {
                    target: _wrapperContainer
                    anchors.leftMargin: _wrapperContainer.tailThicknessMargin
                    anchors.rightMargin: 0
                    anchors.topMargin: 0
                    anchors.bottomMargin: 0
                    anchors.horizontalCenterOffset: 0
            ...
            State {
                ...
                PropertyChanges {
                    target: _wrapperContainer
                    anchors.leftMargin: 0
                    anchors.rightMargin: _wrapperContainer.tailThicknessMargin
                    anchors.topMargin: 0
                    anchors.bottomMargin: 0
                    anchors.horizontalCenterOffset: 0
                    anchors.verticalCenterOffset: 0
        ...
        sourceComponent: LatteComponents.SpriteRectangle {
            ...
            spritePosition: {
                if (root.isHorizontal)
                    return appletItem.parent === appletItem.layouts.startLayout ? PlasmaCore.Types.RightPositioned : PlasmaCore.Types.LeftPositioned;
                else
                    return appletItem.parent === appletItem.layouts.startLayout ? PlasmaCore.Types.BottomPositioned : PlasmaCore.Types.TopPositioned;
            }
        }
