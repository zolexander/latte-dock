...
Item {
    ...
    property bool canShowAppletNumberBadge: !indexerIsSupported && !isSeparator && !isMarginsAreaSeparator && !isHidden && !isSpacer && !isInternalViewSplitter
    readonly property bool canFillScreenEdge: communicator.requires.screenEdgeMarginSupported || communicator.indexerIsSupported
    readonly property bool canFillThickness: applet && applet.hasOwnProperty("constraintHints") && ((applet.constraintHints & PlasmaCore.Types.CanFillArea) === PlasmaCore.Types.CanFillArea)
    readonly property bool isMarginsAreaSeparator: applet && applet.hasOwnProperty("constraintHints") && ((applet.constraintHints & PlasmaCore.Types.MarginAreasSeparator) === PlasmaCore.Types.MarginAreasSeparator)
    readonly property color highlightColor: theme.buttonFocusColor
    //! Fill Applet(s)
    property bool inFillCalculations: false
    //temp record, is used in calculations for fillWidth,fillHeight applets
    ...
    property bool isRequestingFill: {
        ...
        if ((root.isHorizontal && applet.Layout.fillWidth === true) || (root.isVertical && applet.Layout.fillHeight === true))
            return !isHidden;
    ...
    readonly property bool inConfigureAppletsDragging: root.dragOverlay && root.dragOverlay.currentApplet && root.dragOverlay.pressed
    ...
    readonly property bool lockZoom: !parabolicEffectIsSupported || appletBlocksParabolicEffect || (fastLayoutManager && applet && (fastLayoutManager.lockedZoomApplets.indexOf(applet.id) >= 0))
    readonly property bool userBlocksColorizing: appletBlocksColorizing || (fastLayoutManager && applet && (fastLayoutManager.userBlocksColorizingApplets.indexOf(applet.id) >= 0))
    property bool isActive: (isExpanded && !appletItem.communicator.indexerIsSupported && applet.pluginName !== "org.kde.activeWindowControl" && applet.pluginName !== "org.kde.plasma.appmenu")
    ...
    property bool isSeparator: applet && (applet.pluginName === "audoban.applet.separator" || applet.pluginName === "org.kde.latte.separator")
    ...
    property bool firstAppletInContainer: (index >= 0) && ((index === layouter.startLayout.firstVisibleIndex) || (index === layouter.mainLayout.firstVisibleIndex) || (index === layouter.endLayout.firstVisibleIndex))
    //applet is in ending edge
    property bool lastAppletInContainer: (index >= 0) && ((index === layouter.startLayout.lastVisibleIndex) || (index === layouter.mainLayout.lastVisibleIndex) || (index === layouter.endLayout.lastVisibleIndex))
    readonly property bool acceptMouseEvents: applet && !indexerIsSupported && !originalAppletBehavior && parabolicEffectIsSupported && !appletItem.isSeparator && !communicator.requires.parabolicEffectLocked
    ...
    readonly property bool originalAppletBehavior: root.behaveAsPlasmaPanel || !parabolicEffectIsSupported || (root.behaveAsDockWithMask && !parabolicEffectIsSupported) || (root.behaveAsDockWithMask && parabolicEffectIsSupported && lockZoom)
    ...
    property real computeWidth: root.isVertical ? wrapper.width : hiddenSpacerLeft.width + wrapper.width + hiddenSpacerRight.width
    property real computeHeight: root.isVertical ? hiddenSpacerLeft.height + wrapper.height + hiddenSpacerRight.height : wrapper.height
    property var applet: null
    property Item appletItem: null
    property Item latteStyleApplet: null
...
Component.onCompleted: {
    ...
    console.log("LATTE DEBUG: AppletItem completed plugin=", pluginName,
                "applet=", applet,
                "item geom=", "x=", x, "y=", y, "w=", width, "h=", height,
                "wrapper size=", wrapper.width, "x", wrapper.height,
                "appletWidth/Height=", _wrapper.appletWidth, _wrapper.appletHeight);
    delayedSizeLogTimer.start();
}
    ...
    // One-shot timer to log sizes shortly after startup
    Timer {
        id: delayedSizeLogTimer

        interval: 500
        repeat: false
        onTriggered: {
            console.log("LATTE DEBUG: AppletItem delayed sizes plugin=", pluginName,
                        "applet=", applet,
                        "item geom=", "x=", x, "y=", y, "w=", width, "h=", height,
                        "wrapper size=", wrapper.width, "x", wrapper.height,
                        "appletWidth/Height=", _wrapper.appletWidth, _wrapper.appletHeight);
    ...
    Timer {
        ...
        onTriggered: {
            ...
            if (isSystray || !appletItem.isSpacer)
    ...
    Connections {
        ...
        onWheelScrolled: {
            ...
            blockWheel = true;
            scrollDelayer.start();
            if (appletItem.containsPos(pos) && (root.latteView.extendedInterface.appletIsExpandable(applet.id) || (root.latteView.extendedInterface.appletIsActivationTogglesExpanded(applet.id)))) {
                var angle = angleDelta.y / 8;
                var expanded = root.latteView.extendedInterface.appletIsExpanded(applet.id);
                if ((angle > 12 && !expanded) || (angle < -12 && expanded))
                    //positive direction
                    //negative direction
                    latteView.extendedInterface.toggleAppletExpanded(applet.id);
    ...
    Connections {
        ...
        onExpandedAppletStateChanged: {
            if (latteView.extendedInterface.hasExpandedApplet && appletItem.applet)
                appletItem.isExpanded = latteView.extendedInterface.appletIsExpandable(appletItem.applet.id) && latteView.extendedInterface.appletIsExpanded(appletItem.applet.id);
            else
    ...
    Flow {
        ...
        Item {
            ...
            Colorizer.Applet {
                ...
                readonly property bool mustBeShown: colorizerManager.mustBeShown && !appletItem.userBlocksColorizing && !appletItem.appletBlocksColorizing && !appletItem.isInternalViewSplitter
            ...
            Item {
                ...
                readonly property int badgeThickness: {
                    if (plasmoid.location === PlasmaCore.Types.BottomEdge || plasmoid.location === PlasmaCore.Types.RightEdge) {
